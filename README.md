...
## 3  Square Drive
![Square Drive Demo](figures/gazebo_square_demo.gif)
...
## 4  Wall Follow
![Wall Follow Demo](figures/wallfollow.gif)
...
### 4.2  Methodology
![Wall Follow Methodology](figures/4.2%20figure.png)

The lidar data comes in an /LaserScan format, which provides an array of ranges (distance measurements) for evenly spaced angles around the sensor. For the purpose of our wall following, we only parsed data in a smaller window that starts in front of the neato.

#### 4.2.1  Detecting a Wall
![Wall Detection](figures/4.2.1%20figure.png)

The transition to wall following comes from the detect_wall() method returning True. The detect wall method uses the points in the lidar window and fits a linear regression to the points in reference to the robot axis. One of the important values returned from the regression is the coefficient of determination, or R^2. R^2 is a vlaue from 0 to 1, where a value of 1 shows a strong linear fit and a value of 0 shows an extremely poor linear state. 

For our detect_wall() method, we say that a wall will be detected if the r^2 value is above a certain threshold. This ensures that curved objects are not mistake for a wall.

#### 4.2.2  Approaching and Following the Wall
![Wall Approach and Follow](figures/4.2.2%20figure.png)
...
One limitation of our current wall-following implementation is that the robot can sometimes perform a full 360-degree spin when initially trying to detect a wall. Also, our lidar window is designed to rotate to stay perpendicular to the wall, the robot may fail to recognize sharp 90-degree turns. In such cases, the Neato continues driving forward and risks colliding with the corner instead of properly following the wall.

---

## 5  Person Follow
![Person Following Demo](figures/gazebo_person_follow_demo.gif)
...
### 5.2 Methodology
For LIDAR processing, the node receives LIDAR data through a subscription to the "/scan" topic, which provides a 360 degree array of distance measurements. To reduce noise and focus only on relevant information, the data is filtered to include points within a front-facing 90 degree  field of view and within a maximum detection range of 1.5 meters. From the filtered data, each valid LIDAR return is converted from polar to Cartesian coordinates in the robot's local frame. The mean position, or "center of mass," of these points is then computed to represent the target location. This ensures that a single robust waypoint can be identified even when the detected person appears as a cluster of scan points. Since navigation is performed in the global odom frame, this target position is subsequently transformed from the local "base_laser_link" frame into the "odom" (global) frame using TF2 transformation methods. The transformed point serves as a continuously updating target waypoint for the robot to follow.

To drive toward the waypoint, the Neato uses similar methods of proportional control to previous behaviors. It computes its heading and distance errors relative to the transformed target, which are then passed onto the proportional controller that generates linear and angular velocity commands, allowing the robot to smoothly adjust its trajectory. If the robot approaches within 0.75 m of the target, all commanded velocities are set to zero. As soon as the person is detected farther away, the robot automatically resumes movement toward the new waypoint. If the person suddenly steps outside of the LIDAR scan window, the Neato will continue to navigate to the last valid target waypoint until a new person is detected.
...
![Obstacle Avoidance Demo](figures/gazebo_obstacle_avoidance_demo.gif)

### 6.1 Overview
![Obstacle Avoidance Overview](figures/6.1%20figure.png)
...
![FSM States and Transitions](figures/7.1%20figure.png)
...
How the order of topic changes affected by our two nodes is illustrated above.
...
### 7.4  RQT Graph
![RQT Graph](figures/7.4%20figure.png)

Our nodes and topics are shown above.
...
### 8.2 Takeaways
This project gave us meaningful, hands-on experience in building and integrating robotic systems. We came to appreciate the importance of designing a clear node and topic structure so that each behavior communicates reliably and conflicts are avoided. We also learned how easily issues can arise when multiple variables are manipulated at once, which highlighted the need for safety and careful coordination.

Working through unexpected challenges gave us a realistic understanding of what can go wrong in practice and why system architecture matters so much. Finally, we gained valuable experience in testing and debugging, reinforcing the idea that catching edge cases early makes integration smoother and prevents small issues from growing into larger problems
